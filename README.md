...
`Order` is the *operation* side of the comparison trichotomy: `Comparison` (from [`swift-comparison`](https://github.com/swift-molecules/swift-comparison)) names the **result** of a comparison (`.less` / `.equal` / `.greater`); `Order.Comparator<T>` names a **rule** for producing that result for two `T`s. Sorting and predicate code reaches for `Order` when the rule itself is the value being passed around; reaches for `Comparison.\`Protocol\`` when a single type's natural ordering is enough.
...
```swift
dependencies: [
    .package(url: "https://github.com/swift-molecules/swift-order.git", branch: "main"),
]
```
...
|---|---|---|
| [`swift-comparison`](https://github.com/swift-molecules/swift-comparison) | What is the *result* of comparing two values? | `Comparison` (`.less` / `.equal` / `.greater`) and `Comparison.\`Protocol\`` for types with a single natural ordering |
...
| [`swift-property`](https://github.com/swift-molecules/swift-property) | How does a fluent `.<verb>` chain attach to a value without forcing it into a protocol? | `Property<Tag, Base>` with `.Inout` for in-place predicates; `Order` is the phantom tag carried into `Property<Order, Self>.Inout` |
...
|---------|--------|---------|
| `Order` | `Sources/Order/` | The `Order` namespace, all its types, and the `.order` fluent surface. Re-exports `Comparison` so a single `import Order` brings the result types into scope too. |
...
- [`swift-comparison`](https://github.com/swift-molecules/swift-comparison) — provides `Comparison` and `Comparison.\`Protocol\``
- [`swift-property`](https://github.com/swift-molecules/swift-property) — provides `Property<Tag, Base>` and `.Inout`
...
- [swift-comparison](https://github.com/swift-molecules/swift-comparison) — `Comparison` and `Comparison.\`Protocol\``
- [swift-property](https://github.com/swift-molecules/swift-property) — `Property<Tag, Base>` and `Property.Inout`
